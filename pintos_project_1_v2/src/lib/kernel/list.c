...
/* Please put new functions below */

// 0921 added custom comparator for blocked_list, which ever sleeps less gets wake earlier.
bool time_to_wake_comparator(const struct list_elem *a, const struct list_elem *b, void *aux UNUSED){
	return list_entry(a, struct thread, elem)->time_to_wake < list_entry(b, struct thread, elem)->time_to_wake;
}

// 0921 DESCENDING ORDER of priority
...
// ASCENDING ORER of priority, can be used to find max and min in the list
bool lower_priority_comparator(const struct list_elem *a, const struct list_elem *b, void *aux UNUSED){
	return get_priority(list_entry(a, struct thread, elem)) <  get_priority(list_entry(b, struct thread, elem));
}
